...
+'</div>'+
  ...
  // ─── Panel 4: 資料備份 ───
  const autoSaveFileStatus=window._nasFileHandle
    ?'<span style="color:var(--accent2)">已連結：'+window._nasFileHandle.name+'</span>'
    :'<span style="color:var(--text3)">尚未設定存檔路徑</span>';
  const backupDirStatus=window._backupDirHandle
    ?'<span style="color:var(--accent2)">已連結：'+window._backupDirHandle.name+'（備份於 project-erp-backup-1/2/3.db）</span>'
    :'<span style="color:var(--text3)">尚未設定備份資料夾</span>';
  const nasStatusText=window._nasFileHandle
    ?'✅ 已連結存檔：'+window._nasFileHandle.name
    :'尚未連結存檔檔案';
  const p4=
    '<div class="s-section"><div class="s-section-title">自動存檔設定</div>'+
    '<div style="padding:12px 14px;background:var(--bg);border-radius:8px;border-left:3px solid var(--accent);margin-bottom:14px;font-size:12.5px;line-height:1.9">'+
    '每次新增或修改資料後，系統將自動：<br>'+
    '① <strong>覆蓋主要存檔</strong>（指定的 .db 檔案）<br>'+
    '② <strong>建立備份</strong>（指定資料夾，最多保留 3 份循環覆蓋）'+
    '</div>'+
    '<div class="form-row" style="margin-bottom:14px">'+
    '<label style="font-size:12px;font-weight:600;margin-bottom:6px;display:block">主要存檔路徑</label>'+
    '<div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap">'+
    '<button class="btn btn-primary btn-sm" onclick="setupAutoSaveFile()">📄 選擇存檔位置</button>'+
    (window._nasFileHandle?'<button class="btn btn-ghost btn-sm" onclick="clearAutoSaveFile()">取消連結</button>':'')+
    '</div>'+
    '<div style="margin-top:6px;font-size:11.5px" id="autosave-file-status">'+autoSaveFileStatus+'</div>'+
    '</div>'+
    '<div class="form-row" style="margin-bottom:14px">'+
    '<label style="font-size:12px;font-weight:600;margin-bottom:6px;display:block">備份資料夾</label>'+
    '<div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap">'+
    '<button class="btn btn-ghost btn-sm" onclick="setupBackupDir()">📁 選擇備份資料夾</button>'+
    (window._backupDirHandle?'<button class="btn btn-ghost btn-sm" onclick="clearBackupDir()">取消連結</button>':'')+
    '</div>'+
    '<div style="margin-top:6px;font-size:11.5px" id="backup-dir-status">'+backupDirStatus+'</div>'+
    '</div>'+
    '<div style="font-size:11.5px" id="nas-status" style="color:var(--text3)">'+nasStatusText+'</div>'+
    '</div>'+
    '<div class="s-section"><div class="s-section-title">手動存取</div>'+
    '<div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:10px;margin-bottom:12px">'+
    '<button class="btn btn-primary" onclick="saveToNAS()" style="padding:12px 8px">💾 立即存檔</button>'+
    '<button class="btn btn-ghost" onclick="openFromNAS()" style="padding:12px 8px">📂 從檔案讀取</button>'+
    '<button class="btn btn-ghost" onclick="exportDB()" style="padding:12px 8px">📥 下載備份</button>'+
    '</div>'+
    '</div>'+
...
function saveSettings(){
  ...
  updateNoPreview();
  if(window._nasFileHandle)saveToNASAuto();
}
